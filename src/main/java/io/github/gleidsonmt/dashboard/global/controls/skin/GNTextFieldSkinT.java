...
import io.github.gleidsonmt.dashboard.global.controls.textField.FieldFooter;
...
public class GNTextFieldSkinT extends TextFieldSkin {
    ...
    public GNTextFieldSkinT(GNTextField textField) {
        ...
        registerChangeListener(textField.clearActionProperty(), "CLEAR_ACTION");
        ...
        registerChangeListener(textField.fieldTypeProperty(), "FIELD_TYPE");
    ...
    protected void handleControlPropertyChanged(String propertyReference) {
        ...
        } else if ("FIELD_TYPE".equals(propertyReference)) {
            updateFieldType();
        } else if ("LEAD_ICON_TYPE".equals(propertyReference)) {
            updateLeadIcon();
        } else if ("CLEAR_ACTION".equals(propertyReference)) {
            updateClearAction();
        }
    ...
    private void updateFieldType() {
        if (textField.getFieldType().equals(FieldType.OUTLINED)) {
            if(!getChildren().contains(border)) getChildren().add(border);
            getChildren().remove(filledBorder);
            border.toBack();
            pseudoClassStateChanged(FILLED_PSEUDO_CLASS, false);
        } else {
            getChildren().remove(border);
            if(!getChildren().contains(filledBorder)) getChildren().add(filledBorder);
            filledBorder.toBack();
    ...
    private void openFilled() {
        ...
        filledAnimation.getKeyFrames().setAll(
                ...
                new KeyFrame(Duration.millis(50),
                        new KeyValue(filledBorder.widthProperty(), textField.getWidth())
                )
        );
    ...
    protected void layoutChildren(double x, double y, double w, double h) {
        ...
        layoutInArea(filledBorder, x -10, y -10, getSkinnable().getWidth(), getSkinnable().getHeight(), 0, HPos.CENTER, VPos.BOTTOM);
