 ...
 */
package io.github.gleidsonmt.dashboard.module.examples.dataTable;

import io.github.gleidsonmt.dashboard.global.creators.DrawerCreator;
...
import io.github.gleidsonmt.dashboard.global.controls.SizeValidator;
import io.github.gleidsonmt.dashboard.global.controls.ValidatorBase;
import io.github.gleidsonmt.dashboard.global.controls.control.GNTextField;
import io.github.gleidsonmt.dashboard.global.converters.MoneySimpleStringConverter;
import io.github.gleidsonmt.dashboard.global.enhancement.CrudView;
import io.github.gleidsonmt.dashboard.global.popup.Popup;
...
public class RegisterController implements Initializable, ActionView, CrudView {
    ...
    @FXML private GNTextField tfName;
    @FXML private GNTextField tfLastName;
...
//                content.getChildren().remove(config);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        validators =  Arrays.asList(
                    new SizeValidator(tfName, "The name of " +
                            tfName.getText() + " is not valid", 3, 28),

                    new SizeValidator(tfLastName, "The last name of " +
                            tfLastName.getText() + " is not valid", 3, 28)

//                    new LetterValidator(tfName, "The size not contains necessary letters")
        );

        tfName.setMaxLength(50D);
        tfLastName.setMaxLength(50D);
        tfName.setVisibleCount(true);
        tfLastName.setVisibleCount(true);

//        validators.forEach(ValidatorBase::reset);
//
...
//        tfName.setSkin(new GNTextFieldSkin(tfName, professional, "Error on this label"));
    ...
    @Override
    public void onEnter() {
        ConfigApp.INSTANCE.getDecorator().widthProperty().addListener(resizeForm);
    ...
    @Override
    public void onExit() {
        ConfigApp.INSTANCE.getDecorator().widthProperty().removeListener(resizeForm);
    }
    ...
    private void openAvatarPopup() throws NavigationException {
        if(ConfigApp.INSTANCE.getDecorator().getWidth() < 800) {
